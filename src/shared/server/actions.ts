...
import {
  ...
  getDocumentList,
  getPriceListByName,
  getPriceListByNameAndMarket,
  getPriceListWithPagination,
  Price,
} from '@/shared/service/firebase';
import {
  createInvoice,
  createItemList,
  createMarket,
  createVirtualDocument,
} from '@/shared/util/sefaz';
...
export async function getPricesByName(query: string, market?: string) {
  try {
    const list = market
      ? await getPriceListByNameAndMarket(query, market)
      : await getPriceListByName(query);
    return {
      status: 200,
      data: list,
    };
...
export async function getListUrlInvoice() {
  try {
    const list = (await getDocumentList('notaFiscal')).map(
      (invoice) => invoice.url
    );
    return {
      status: 200,
