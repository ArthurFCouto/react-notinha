...
export default function CardItems({ items, clickOnHistory }: CardItemsProps) {
  const transitionValue = (value: number) =>
    value > 20 ? '1000ms' : `${value * 50}ms`;
...
const createNumbersArray = (amount: number) => {
  const array = [];
  for (let i = 0; i < amount; i++) array.push(i);
  return array;
};
...
export function CardItemsLoading({ amount }: Readonly<{ amount: number }>) {
  const LoadingSkeleton = () => (
    <ListItem alignItems="flex-start">
      ...
      <ListItemText
        primary={
          <Skeleton animation="wave" sx={{ fontSize: '1rem' }} variant="text" />
        }
        secondary={<Skeleton animation="wave" variant="rectangular" />}
      />
    </ListItem>
  );
  return (
    <List sx={{ width: '100%' }}>
      {createNumbersArray(amount).map((index) => (
