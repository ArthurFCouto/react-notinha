...
export function HandleResponseNF(response: string) {
    const parse = new DOMParser();
    // Transforma  uma string com código html em um documento manipulável
    const virtualDoc = parse.parseFromString(response, 'text/html');
    // Encontrando a tabela que contém a lista dos produtos
    const tabela = virtualDoc.querySelector('.table.table-striped');
    // Verificar se a tabela foi encontrada
    if (tabela) {
        HandleCNPJ(virtualDoc);
        HandleNF(virtualDoc);
